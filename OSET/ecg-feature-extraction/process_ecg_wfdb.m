function ecg_features_vector = process_ecg_wfdb(input_wfdb_address, csv_path , lead_names_target, window_dur, start_time, stop_time, flatten_flag)
...
for n = 1:num_windows
    ...
    parfor c = 1:C
        try
            [ecg_features_vector_ch{1,c}, ecg_feature_info, ecg_fiducial_position, ~] = ...
                ecg_feature_extraction(win_ecg_data(c,:), fs, lead_names, [], [], [], [], flatten_flag);
        catch
            ecg_features_vector_ch{1,c} = nan(1,118);
        end

    end

    ecg_features_vector = cell2mat(ecg_features_vector_ch);
